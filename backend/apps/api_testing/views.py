...
class ApiRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if project_id:
            # 如果指定了项目，则只查询该项目下的接口（通过集合关联）
            queryset = queryset.filter(
                collection__project_id=project_id
            )
    ...
    def execute(self, request, pk=None):
        ...
        try:
            ...
            if isinstance(params, list):
                # 列表形式转换为字典
                params = {item.get('key', ''): item.get('value', '') for item in params if item.get('key')}
            else:
            ...
            if request_body and request_method in ['POST', 'PUT', 'PATCH']:
                ...
                if body_type == 'json':
                    ...
                    if isinstance(body_content, str):
                        ...
                        body_data = body_content
                elif body_type == 'multipart-formdata':
                    # multipart/form-data: 文本字段 + 文件字段
                    form_data = {}
                        ...
                        for item in body_content:
                            if isinstance(item, dict) and item.get('key'):
                                ...
                                if item.get('type') == 'file' and item.get('value'):
                                    # 文件字段：value 可能是文件路径或 base64
                                    file_path = str(item['value'])
                                    if os.path.isfile(file_path):
                                        form_data[key] = (os.path.basename(file_path), open(file_path, 'rb'), item.get('content_type', 'application/octet-stream'))
                                    elif file_path.startswith('/media/'):
                                        media_path = os.path.join(settings.MEDIA_ROOT, file_path.lstrip('/media/'))
                                        if os.path.isfile(media_path):
                                            form_data[key] = (os.path.basename(media_path), open(media_path, 'rb'), item.get('content_type', 'application/octet-stream'))
                                else:
                                    val = self._replace_variables(str(item.get('value', '')), variables)
                                    form_data[key] = resolver.resolve(val)
                    body_data = form_data
                    body_type = 'files'  # 标记为文件上传模式
                elif body_type == 'form-data':
                    if isinstance(body_content, list):
                        body_data = {item.get('key', ''): self._replace_variables(str(item.get('value', '')), variables) for item in body_content if item.get('key')}
                        for key in body_data:
                            body_data[key] = resolver.resolve(str(body_data[key]))
                    else:
                        body_data = body_content
                    ...
                    # 二进制文件上传
                    if isinstance(body_content, str) and os.path.isfile(body_content):
                        with open(body_content, 'rb') as f:
                            body_data = f.read()
                    elif isinstance(body_content, str) and body_content.startswith('/media/'):
                        media_path = os.path.join(settings.MEDIA_ROOT, body_content.lstrip('/media/'))
                        if os.path.isfile(media_path):
            ...
            # 执行 Pre-request Script（Python 沙箱）
            pre_script = request.data.get('pre_request_script', api_request.pre_request_script) or ''
            if pre_script.strip():
                try:
                    script_vars = {
                        'variables': variables,
                    ...
                    exec(pre_script, {'__builtins__': {
                        'True': True, 'False': False, 'None': None,
                        'int': int, 'float': float, 'str': str, 'bool': bool,
                        'list': list, 'dict': dict, 'len': len, 'range': range,
                        'print': lambda *a: logger.info(' '.join(str(x) for x in a)),
                        'json': json, 're': __import__('re'), 'base64': __import__('base64'),
                        'hashlib': __import__('hashlib'), 'datetime': __import__('datetime'),
                        'random': __import__('random'), 'uuid': __import__('uuid'),
                    }}, script_vars)
                    # 提取修改后的值
                    variables = script_vars.get('variables', variables)
                    headers.update(script_vars.get('headers', {}))
                    body_data = script_vars.get('body', body_data)
                    params = script_vars.get('params', params)
                    url = script_vars.get('url', url)
                except Exception as script_err:
                    logger.warning(f"Pre-request script execution error: {script_err}")
            ...
            if body_type == 'files':
                ...
                response = requests.request(
                    ...
                    headers=headers,
                    params=params,
                    data=body_data if isinstance(body_data, bytes) else str(body_data).encode('utf-8'),
                    timeout=30
                )
            else:
            ...
            if hasattr(response, 'cookies'):
                for cookie in response.cookies:
                    response_cookies.append({
                        ...
                        'path': cookie.path or '/',
                        'expires': str(cookie.expires) if cookie.expires else '',
                        'secure': bool(cookie.secure),
                        'http_only': bool(getattr(cookie, 'has_nonstandard_attr', lambda x: False)('httponly') or False),
                    })

            # 执行 Post-request Script（Python 沙箱，用于响应验证和变量提取）
            post_script = request.data.get('post_request_script', api_request.post_request_script) or ''
            script_results = {}
            if post_script.strip():
                try:
                    ...
                    script_results = post_vars.get('script_results', {})
                except Exception as script_err:
                    logger.warning(f"Post-request script execution error: {script_err}")
                    script_results = {'error': str(script_err)}
            ...
            history = RequestHistory.objects.create(
                ...
                request_data={
                    ...
                    'headers': headers,
                    'params': params,
                    'body': str(body_data)[:5000] if isinstance(body_data, bytes) else body_data
                },
                response_data={
            ...
            history_data['response_data']['cookies'] = response_cookies
            history_data['script_results'] = script_results
