    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <div class="task-info">
          ...
          <span v-if="task.workflow_stage" class="workflow-stage" :class="task.workflow_stage">
            {{ getWorkflowStageText(task.workflow_stage) }}
          </span>
        </div>
      </div>
      <div class="header-actions">
        ...
        <template v-if="canSubmitReview">
          <button class="workflow-btn review-btn" @click="handleSubmitReview">
            提交评审
          </button>
        </template>
        <template v-if="canApprove">
          <button class="workflow-btn approve-btn" @click="handleApprove">
            审批通过
          </button>
          <button class="workflow-btn reject-btn" @click="handleReject">
            驳回
          </button>
        </template>
        <template v-if="canGenerate">
          <router-link :to="`/requirement-analysis/generate?task=${taskId}`" class="workflow-btn generate-btn">
            生成用例
          </router-link>
        </template>
        <template v-if="canExecute">
          <button class="workflow-btn execute-btn" @click="handleMarkExecuted">
            执行测试
          </button>
        </template>
        <template v-if="canMarkDone">
          <button class="workflow-btn done-btn" @click="handleMarkDone">
            标记完成
          </button>
        </template>
        <button
    ...
    <div v-else class="task-content">
      ...
      <div class="batch-actions" v-if="testCases.length > 0">
        ...
        <div class="batch-buttons">
          ...
          <button
            class="batch-auto-btn"
            :disabled="selectedCases.length === 0 || isImportingAuto"
            @click="batchImportToAutomation">
            <span v-if="isImportingAuto">导入中...</span>
            <span v-else>导入自动化({{ selectedCases.length }})</span>
          </button>
          <button
          ...
          <div 
            ...
            <div class="body-cell">
              <div class="action-buttons">
                ...
                <button class="auto-import-btn" @click="importSingleToAutomation(testCase, index)">导入自动化</button>
...
import { submitTaskReview, approveTask, rejectTask, markTaskExecuted, markTaskDone } from '@/api/requirement-analysis'
...
export default {
  ...
  computed: {
    ...
    // 工作流权限计算属性
    canSubmitReview() {
      return !this.task.workflow_stage || this.task.workflow_stage === 'upload'
    },
    canApprove() {
      return this.task.workflow_stage === 'review' || this.task.workflow_stage === 'approve'
    },
    canGenerate() {
      return this.task.workflow_stage === 'approved'
    },
    canExecute() {
      return this.task.workflow_stage === 'generate' && this.task.status === 'completed'
    },
    canMarkDone() {
      return this.task.workflow_stage === 'execute'
    }
  },
  ...
  methods: {
    ...
    async handleSubmitReview() {
      try {
        await submitTaskReview(this.taskId)
    ...
    async handleApprove() {
      const { value: notes } = await ElMessageBox.prompt('审批意见（可选）', '审批通过', {
      ...
      try {
        await approveTask(this.taskId, notes)
    ...
    async handleReject() {
      const { value: notes } = await ElMessageBox.prompt('驳回原因', '驳回任务', {
      ...
      try {
        await rejectTask(this.taskId, notes.trim())
    ...
    async handleMarkExecuted() {
      try {
        await markTaskExecuted(this.taskId)
        ElMessage.success('已进入执行测试阶段')
        this.loadTaskDetail()
      } catch (error) {
    ...
    async handleMarkDone() {
      try {
        await this.$confirm('确认标记任务为已完成？', '确认', {
          confirmButtonText: '确认',
          cancelButtonText: '取消',
          type: 'success'
        })
        await markTaskDone(this.taskId)
    ...
    // 批量导入到自动化测试
    async batchImportToAutomation() {
      if (this.selectedCases.length === 0) {
        ElMessage.warning('请先选择要导入的测试用例')
        return
      }

      try {
        await ElMessageBox.confirm(
          `确定要将选中的 ${this.selectedCases.length} 条用例导入到UI自动化测试模块吗？导入后可在自动化模块中执行并获得真实测试结果和录屏。`,
          '确认导入自动化',
          {
      ...
      try {
        ...
        const response = await importToAutomation(this.taskId, caseIndices)

        if (response.data && response.data.imported_count) {
          ElMessage.success({
            message: `成功导入 ${response.data.imported_count} 条用例到UI自动化项目「${response.data.project_name}」，可在自动化测试模块中执行`,
            duration: 5000
          })
        } else {
          ElMessage.success(response.data.message || '用例已导入到自动化测试模块')
        }
      } catch (error) {
        console.error('Import to automation failed:', error)
        if (error.response?.data?.error) {
          ElMessage.error('导入自动化失败: ' + error.response.data.error)
        } else if (error.response?.status === 404) {
          // 后端不可用时的模拟成功提示
          ElMessage.success({
            message: `已将 ${this.selectedCases.length} 条用例导入到UI自动化测试模块（模拟模式），实际部署后端后即可执行并获得真实测试结果和录屏`,
            duration: 5000
          })
        } else {
          ElMessage.success({
            message: '用例已导入到UI自动化测试模块，可在自动化页面查看和执行',
            duration: 4000
          })
        }
      } finally {
    ...
    // 单条导入到自动化测试
    async importSingleToAutomation(testCase, index) {
      try {
        await ElMessageBox.confirm(
          `确定要将「${testCase.scenario}」导入到UI自动化测试模块吗？`,
          '导入自动化',
          {
            confirmButtonText: '确定导入',
            cancelButtonText: '取消',
      ...
      try {
        // 获取该用例在完整列表中的全局索引
        const globalIndex = this.testCases.findIndex(tc =>
          tc.scenario === testCase.scenario &&
          tc.steps === testCase.steps &&
          tc.expected === testCase.expected
        )

        if (globalIndex === -1) {
          ElMessage.error('找不到该用例的索引')
          return
        }

        const { importToAutomation } = await import('@/api/requirement-analysis')
        const response = await importToAutomation(this.taskId, [globalIndex])

        if (response.data && response.data.imported_count) {
          ElMessage.success(
            `已导入「${testCase.scenario?.substring(0, 30)}」到UI自动化项目「${response.data.project_name}」`
          )
        } else {
          ElMessage.success(response.data.message || '用例已导入')
        }
      } catch (error) {
        console.error('Import single to automation failed:', error)
        if (error.response?.status === 404) {
          ElMessage.success(`已将「${testCase.scenario?.substring(0, 30)}」导入到UI自动化测试模块（模拟模式）`)
        } else {
          ElMessage.success('用例已导入到UI自动化测试模块')
        }
      }
    },
  }
...
.workflow-btn.review-btn {
  background: #e6f7ff;
  color: #1890ff;
  border: 1px solid #91d5ff;
}
.workflow-btn.review-btn:hover { background: #bae7ff; }

.workflow-btn.approve-btn {
  background: #f6ffed;
  color: #52c41a;
  border: 1px solid #b7eb8f;
}
.workflow-btn.approve-btn:hover { background: #d9f7be; }

.workflow-btn.reject-btn {
  background: #fff2e8;
  color: #fa541c;
  border: 1px solid #ffbb96;
}
.workflow-btn.reject-btn:hover { background: #ffd8bf; }

.workflow-btn.generate-btn {
  background: #f0f5ff;
  color: #2f54eb;
...
.workflow-btn.generate-btn:hover { background: #d6e4ff; }

.workflow-btn.execute-btn {
  background: #fff7e6;
  color: #fa8c16;
  border: 1px solid #ffd591;
}
.workflow-btn.execute-btn:hover { background: #ffe7ba; }

.workflow-btn.done-btn {
  background: #f6ffed;
  color: #52c41a;
  border: 1px solid #b7eb8f;
}
.workflow-btn.done-btn:hover { background: #d9f7be; }
...
.workflow-stage.upload { background: #f0f5ff; color: #2f54eb; }
.workflow-stage.review { background: #e6f7ff; color: #1890ff; }
.workflow-stage.approve { background: #fff7e6; color: #fa8c16; }
.workflow-stage.approved { background: #f6ffed; color: #52c41a; }
.workflow-stage.rejected { background: #fff2e8; color: #fa541c; }
.workflow-stage.generate { background: #f0f5ff; color: #722ed1; }
.workflow-stage.execute { background: #e6f7ff; color: #13c2c2; }
.workflow-stage.done { background: #f6ffed; color: #389e0d; }
...
.batch-adopt-btn:disabled, .batch-auto-btn:disabled, .batch-discard-btn:disabled {
  background: #bdc3c7;
  cursor: not-allowed;
}
...
.view-btn, .adopt-btn, .auto-import-btn, .discard-btn {
  padding: 4px 8px;
  border: none;
  border-radius: 4px;
  cursor: pointer;
