      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <form @submit.prevent="saveConfig">
            ...
            <div class="form-group">
              <label>自动化模型角色 <span class="required">*</span></label>
              <select v-model="configForm.role" class="form-select" required>
                <option value="browser_use_text">网页自动化文本模型</option>
                <option value="browser_use_vision">网页自动化视觉模型</option>
              </select>
              <small class="form-hint">
                文本模型用于 DOM/步骤推理，视觉模型用于截图识别；可同时启用多个模型，后端按可用性自动选择。
              </small>
            </div>
...
const getRoleLabel = (role) => {
  const roleMap = {
    browser_use_text: '文本自动化',
    browser_use_vision: '视觉自动化'
  }
  return roleMap[role] || role || '文本自动化'
}
